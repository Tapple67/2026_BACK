...
SELECT 제품명, 카테고리명 FROM product p1 JOIN pcategory p2 on p1.카테고리번호_fk = p2.카테고리번호_pk;

-- [문제 2] '노트북' 카테고리에 속하는 모든 제품의 제품명과 제품가격을 조회하세요. 
SELECT 제품명,제품가격 FROM product p1 JOIN pcategory p2 on p1.카테고리번호_fk = p2.카테고리번호_pk WHERE 카테고리명 ="노트북";

-- [문제 3] 모든 제품의 제품명과 등록된 재고수량을 함께 조회하세요. 
SELECT 제품명,재고수량 FROM product p1 JOIN stock c2 on p1.제품번호_fk = c2.제품번호_pk

-- [문제 4] '그램 15인치' 제품의 모든 재고등록날짜와 재고수량을 조회하세요.
SELECT 제품명, 재고등록날짜, 재고수량 FROM product p1 JOIN stock c2 on p1.제품번호_fk = c2.제품번호_pk WHERE 제품명="그램 15인치";

-- [문제 5] 모든 제품의 제품명, 카테고리명, 재고수량을 한 번에 조회하세요. (3개 테이블 조인)
SELECT 제품명, 카테고리명, 재고수량 FROM product JOIN stock on 제품번호_fk = 제품번호_pk JOIN pcategory on 카테고리번호_fk = 카테고리번호_pk;

-- [문제 6] 모든 카테고리의 카테고리명과 해당 카테고리에 속한 제품명을 조회하세요. 만약 카테고리에 속한 제품이 없더라도 카테고리명은 모두 표시되도록 하세요.
SELECT 카테고리명, 제품명 FROM product p1 RIGHT JOIN pcategory p2 on 카테고리번호_fk = 카테고리번호_pk;

-- [문제 7] 재고가 한 번도 등록되지 않은 제품의 제품명을 조회하세요. 
SELECT 제품명 FROM product p1 LEFT JOIN stock c2 on 제품번호_fk = 제품번호_pk WHERE 제품번호_fk IS NULL;


-- [문제 8] 각 카테고리별로 총 재고 수량의 합계를 카테고리명과 함께 조회하세요. 
SELECT 카테고리명, sum(재고수량) 합계 FROM product JOIN stock on 제품번호_fk = 제품번호_pk JOIN pcategory on 카테고리번호_fk = 카테고리번호_pk GROUP BY 카테고리명;

-- [문제 9] 각 제품별로 총 재고 수량을 조회하고, 총 재고 수량이 많은 순서대로 정렬하여 제품명과 총재고수량을 표시하세요.
SELECT 제품명, sum(재고수량) 총재고수량 FROM product JOIN stock on 제품번호_fk = 제품번호_pk GROUP BY 제품명 ORDER BY 총재고수량 desc;