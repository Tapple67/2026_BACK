...
SELECT DISTINCT customer 고객 FROM orders  GROUP BY customer;


-- [문제 2 ] books 테이블에서 장르(genre) 목록을 중복 없이 조회하는 SQL을 작성하세요.
SELECT DISTINCT genre 장르 FROM books;

-- [문제 3 ] orders 테이블에서 도서번호(book_id)별 주문이 존재하는 도서 목록을 조회하세요.
SELECT title FROM books WHERE book_id IN (SELECT book_id FROM orders WHERE book_id IS NOT NULL);
...
SELECT book_id 도서번호, sum(order_qty) FROM orders GROUP BY book_id;

-- [문제 12 ] orders에서 고객별 총 주문수량 합계가 5 이상인 고객만 조회하세요. ( 출력: customer, 총주문수량, 조건: HAVING 사용 )
SELECT customer,sum(order_qty) 총주문수량 FROM orders GROUP BY customer HAVING sum(order_qty) >= 5;

-- [문제 13 ] orders에서 도서번호(book_id)별 주문 건수가 3회 이상인 도서만 조회하세요.( 출력: book_id, 주문건수 )
SELECT book_id, sum(order_qty) 주문건수 FROM orders GROUP BY book_id HAVING order_qty >=3;
